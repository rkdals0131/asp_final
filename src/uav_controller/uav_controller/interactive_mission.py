...
class InteractiveMissionNode(BaseMissionNode):
    ...
    def __init__(self):
        ...
        # --- 자동 핸드셰이크 시작 타이머 ---
        self.create_timer(0.5, self._auto_start_handshake_if_ready)
    ...
    def _auto_start_handshake_if_ready(self):
        """INIT 상태에서 대기하다가 첫 번째 local position 수신 시 HANDSHAKE로 전환"""
        if self.state == "INIT" and self.current_local_pos:
