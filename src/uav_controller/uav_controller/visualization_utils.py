...
def create_waypoint_visual(node, waypoint_id, position, color, text_label="", ns_prefix="waypoint"):
    """
    ...
    cylinder_marker = create_marker(
        node, "map", f"{ns_prefix}_cylinders", waypoint_id,
...
def create_mission_visual_markers(node, waypoints, stare_targets=None, current_waypoint_idx=0):
    ...
    for i, wp in enumerate(waypoints):
        ...
        wp_markers = create_waypoint_visual(node, i, wp, color, text_label, "waypoints")
    ...
    if stare_targets:
        for i, target in enumerate(stare_targets):
            ...
            target_markers = create_waypoint_visual(node, i, target, color, text_label, "stare_targets")
...
def create_interactive_mission_markers(node, waypoints, final_destination=None):
    ...
    for i, waypoint in enumerate(waypoints):
        ...
        markers = create_waypoint_visual(node, i, waypoint, color, text_label, "waypoint")
        marker_array.markers.extend(markers)
    
    # 최종 목적지 (있는 경우)
    if final_destination:
        gold_color = (1.0, 0.8, 0.0)  # 골드색
        final_text = f"FINAL\n({final_destination[0]:.2f}, {final_destination[1]:.2f}, {final_destination[2]:.2f})"
        
        final_markers = create_waypoint_visual(node, 0, final_destination, gold_color, final_text, "final")
